...
from person_verification.Src.main_algorithm.Modules.mydeepface.basemodels import (
    VGGFace,
    OpenFace,
    Facenet,
    Facenet512,
...
from person_verification.Src.main_algorithm.Modules.mydeepface.extendedmodels import Age, Gender, Race, Emotion
from person_verification.Src.main_algorithm.Modules.mydeepface.commons import functions, realtime, distance as dst
...
class find_config:
    def __init__(self, arg_representations, device):
    ...
    def find(
        ...
        file_name = f"representations_{model_name}.json"
        ...
        if db_path != False:
            if path.exists(db_path + "/" + file_name):
                ...
                myrepresentations = json.dumps(representations)
                with open(f"{db_path}/{file_name}", "w") as outfile:
                    outfile.write(myrepresentations)

        else:
            representations = self.arg_representations
...
def detectFace(
    img_path, device, target_size=(224, 224), detector_backend="opencv", enforce_detection=True, align=True
):
    """
