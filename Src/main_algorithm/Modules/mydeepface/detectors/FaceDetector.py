...
import numpy as np
from person_verification.Src.main_algorithm.Modules.mydeepface.commons import distance
from person_verification.Src.main_algorithm.Modules.mydeepface.detectors import (
    OpenCvWrapper,
    SsdWrapper,
    DlibWrapper,
    MtcnnWrapper,
...
def detect_faces(face_detector, detector_backend, img, device, align=True):
    backends = {
    ...
    if detect_face_fn:  # pylint: disable=no-else-return
        obj = detect_face_fn(face_detector, img, align, device)
