from person_verification.Src.main_algorithm.Modules.mydeepface.detectors import FaceDetector
import torch
...
def build_model():
    ...
    from person_verification.Src.main_algorithm.Modules.mydeepface.commons.functions import get_deepface_home
...
def detect_face(face_detector, img, device, align=False):
    resp = []
    ...
    results = face_detector.predict(
        img, verbose=False, show=False, conf=0.25, device=device)[0]
